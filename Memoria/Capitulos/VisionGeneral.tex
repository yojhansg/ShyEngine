...
Por lo tanto, hemos optado por desarrollar un motor dirigido por datos específicamente diseñado para juegos 2D simples y eficientes, complementado por un editor que permita la creación de estos juegos. En nuestro enfoque, los datos desempeñan un papel central al servir como el medio de comunicación entre el editor y el motor. El motor genera datos sobre los componentes disponibles para el editor y el editor genera datos sobre el juego que el motor puede leer e interpretar.
...
Como se mencionó en el apartado de motivación [\ref{cap1:sec:motivacion}], el objetivo es desarrollar un motor con editor que pudiera ser utilizado por gente que no sea programadora. Por ello, se ha decidido implementar un sistema de programación visual. 
...
Este fichero formará parte de los recursos del juego, y será leído por el motor durante la ejecución del juego. En el proceso de lectura, el motor leerá y procesará esta información, almacenándolo en una estructura de datos que representa el script y que puede ser accedida más adelante.

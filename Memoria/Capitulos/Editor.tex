...
El flujo de trabajo del editor comienza con la ventana del gestor de proyectos. Después se traslada a una pila de estados, que determina qué ventana se debe mostrar y manejar en un momento dado, ya sea la ventana de el editor principal o edición de scripts.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Vectorial/FuncionamientoEditor.png}
...
La ventana del editor principal cuenta con varias subventanas:
...
Aunque también dispone de otras subventanas más secundarias como la de preferencias, la de gestión de prefabs, la de edición de la paleta de colores, o la consola.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Vectorial/ArquitecturaVentanas.png}
...
Cabe destacar que el editor también soporta la modificación del layout de ventanas y personalización de la paleta de colores, como se ve en la figura~\ref{fig:personalizacionEditor}.
...
Como se mencionó anteriormente en la sección~\ref{generacionDeDatos}, el motor del cuenta con una herramienta que serializa en formato JSON los nombres de los atributos correspondientes a sus componentes nativos.
\medskip

Los datos en formato JSON se interpretan y se utilizan para ser editados desde la ventana de Componentes de ShyEditor.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{Imagenes/Vectorial/LecturaComponentesEditor.png}
...
Como hemos mencionado anteriormente, el editor contiene una ventana completamente dedicada a la creación y modificación de scripts. 

\medskip

En el editor, los scripts se crean utilizando un sistema de programación visual (sección~\ref{programacionVisual}) mediante nodos y conexiones. Los nodos representan acciones o eventos (sección~\ref{comunicacionScripting}) específicos que ocurren en el juego, como el inicio de una escena, la colisión entre objetos o la activación de una trampa. Estos nodos se organizan en el espacio de trabajo del editor y se conectan entre sí para definir la secuencia y lógica del comportamiento.

%---------------------------------------------------------------------
%
%                          Contribuciones
%
%---------------------------------------------------------------------

\chapter{Videojuegos de ejemplo}

Durante el proceso de desarrollo de nuestro motor de videojuegos y editor, hemos tenido la oportunidad de aplicar estas herramientas en la creación de varios videojuegos. Estos proyectos nos han permitido poner a prueba el potencial y la versatilidad de nuestro motor, así como demostrar su capacidad para crear una variedad de experiencias de juego. A continuación, destacamos algunos de los videojuegos que hemos desarrollado utilizando nuestras propias herramientas:

\section{Space Invaders}

Uno de nuestros primeros proyectos fue una versión simplificada del juego Space Invaders, este juego nos permitió poner a prueba cosas como los cambios de escenas, instanciacion de prefabs, manejo del input del jugador y las colisiones a través del scripting. 

\medskip

En la imagen \ref{fig:spaceInvaders} se puede observar una captura del juego, y en \url{https://www.youtube.com/watch?v=DOdN1IPDAzQ} un video de ejemplo.


\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{Imagenes/Vectorial/SpaceInvaders.png}
    \caption{Space Invaders desarrollado con ShyEngine.}
    \label{fig:spaceInvaders}
\end{figure}


\section{Plataformas genérico}

También hemos desarrollado un pequeño juego de plataformas que nos permitió usar animaciones, sistema de partículas y un movimiento de plataformeo.

\medskip

En la imagen \ref{fig:plataformeo} se puede observar una captura del juego.

\begin{figure}[h]
    \centering
    \includegraphics[scale=0.8]{Imagenes/Vectorial/Plataformas.png}
    \caption{Juego de plataformas genérico desarrollado con ShyEngine.}
    \label{fig:plataformeo}
\end{figure}